...
router.post("/api/v1/orderReport", (req, res) => {
    ...
    if (accountTransaction.permission === 0) return res.status(401).json({status: "error", data: "Permission violation"});
    ...
    try {
        ...
        const countTransaction = db.prepare(`SELECT COUNT(id) from purchase_history ${filter.length >= 1 ? filterSQL : ""}`).get();
